...
public class Server {
		...
		if (plate.length() == maxLength) {
			...
			if (plate.charAt(2) == '-' && plate.charAt(5) == '-') {
				for (int i = 0; i < maxLength; i = i + 3) {
					if ('A' <= plate.charAt(i) && 'Z' >= plate.charAt(i)) {// for
						// first
						// char
						// of
						// the
						...
						if ('A' <= plate.charAt(i + 1)
								&& 'Z' >= plate.charAt(i + 1)) {// for second
							// char of the
							// field
							fieldLetter++;
						}
					} else if ('0' <= plate.charAt(i) && '9' >= plate.charAt(i)) {
						if ('0' <= plate.charAt(i + 1)
								&& '9' >= plate.charAt(i + 1)) {
							fieldNumber++;
						}
					}
			...
			IOException 
	{
		...
		if (args.length < 1) {
			System.out
					.println("invalid usage. try: java Server <port_number>  ");
			return;
		}
		System.out.println(args);
		// TODO verificar argumentos incluindo se no args[2] é um int

		Server server = new Server(Integer.parseInt(args[0].toString()));
		byte[] receiveBuffer = new byte[server.getBufferMAXSize()];
		DatagramPacket rPacket = new DatagramPacket(receiveBuffer,
		...
		while (true) {
			...
			/*
			 * Scanner a = new Scanner(System.in); String input = a.nextLine();
			 * //to close aplication if(input.equals("q")|| input.equals("Q")) {
			 * //closing socket a.close(); System.out.println("Exiting"); break;
			 * }
			 */

			server.getSocket().receive(rPacket);
			System.out.println("Recebi, a fazer parsing");
			// processing Client's anwser and do the parse
			String request = new String(rPacket.getData());

			Parser parsed = new Parser(request);
			String plate = new String(parsed.getOperands().get(0));
			...
			DatagramPacket sPacket = new DatagramPacket(msg.getBytes(),
					msg.getBytes().length, rPacket.getAddress(),
					rPacket.getPort());
